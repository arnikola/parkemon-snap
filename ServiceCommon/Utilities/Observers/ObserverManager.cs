...
namespace ServiceCommon.Utilities.Observers
{
    ...
    public class ObserverManager<TAddress, TObserver> : IEnumerable<TObserver>
    {
        ...
        public ObserverManager(TimeSpan expiration, Logger log, string logPrefix)
        {
        ...
        /// </summary>
        public IDictionary<TAddress, TObserver> Observers => this.observers.ToDictionary(_ => _.Key, _ => _.Value.Observer);

        /// <summary>
        /// Removes all observers.
        /// </summary>
        public void Clear() => this.observers.Clear();
        ...
        public void Subscribe(TAddress address, TObserver observer)
        {
            ...
            if (this.observers.TryGetValue(address, out entry))
            {
                ...
                this.log.Verbose(this.logPrefix + ": Updating entry for {0}/{1}. {2} total subscribers.", address, observer, this.observers.Count);
            }
            else
            {
                this.observers[address] = new ObserverEntry { LastSeen = now, Observer = observer };
                this.log.Verbose(this.logPrefix + ": Adding entry for {0}/{1}. {2} total subscribers after add.", address, observer, this.observers.Count);
            }
        }
        ...
        public void Unsubscribe(TAddress subscriber)
        {
            ...
            this.log.Verbose(this.logPrefix + ": Removed entry for {0}. {1} total subscribers after remove.", subscriber, this.observers.Count);
        ...
        public async Task Notify(Func<TObserver, Task> notification, Func<TObserver, bool> predicate = null)
        {
            ...
            if (defunct != default(List<TAddress>))
            {
                foreach (var observer in defunct)
                {
                    ...
                    this.log.Verbose(this.logPrefix + ": Removing defunct entry for {0}. {1} total subscribers after remove.", observer, this.observers.Count);
                }
            }
        }
        ...
        /// </returns>
        public IEnumerator<TObserver> GetEnumerator() => this.observers.Select(observer => observer.Value.Observer).GetEnumerator();
        ...
        public TObserver Get(TAddress id)
        {
            TObserver result;
            this.Observers.TryGetValue(id, out result);
            return result;
        }
    }
}
