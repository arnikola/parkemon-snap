namespace Deploy
{
    ...
    {
        /// <summary>
        /// The number of previous versions of an application to retain.
        /// </summary>
        private const int NumRetainedVersions = 5;
        ...
        /// <summary>
        /// The amount of time in seconds to wait between deployment attempts.
        /// </summary>
        private const int RetryWait = 15;

        /// <summary>
        /// The name of the deployment configuration file.
        /// </summary>
        private const string DeploymentConfigFileName = "DeploymentConfig.json";

        /// <summary>
        /// The name of the applications container.
        /// </summary>
        private const string ContainerName = "applications";
        ...
        /// <summary>
        /// The options used in requests to the storage service.
        /// </summary>
        ...
        public async Task<DeploymentConfig> GetDeploymentConfig(CancellationToken cancellationToken)
        {
            ...
            catch (StorageException ex)
            {
                ...
                Console.WriteLine($"Warning, {DeploymentConfigFileName} not found at {blob.Uri}, returning default configuration.");
            ...
            List<string> otherAppNames = null)
        {
            ...
            await this.DeleteOldVersions(application.Id, NumRetainedVersions, cancellationToken);
        }
        ...
        /// </summary>
        /// <param name="appName">The application name.</param>
        /// <param name="numRetainedVersions">
        /// The number of versions to retain.
        /// </param>
        /// <param name="cancellationToken">The cancellation token.</param>
        /// <returns>A <see cref="Task"/> representing the work performed.</returns>
        private async Task DeleteOldVersions(string appName, int numRetainedVersions, CancellationToken cancellationToken)
        {
            var directories = await this.GetVersionedDirectories(appName, cancellationToken);
            if (directories.Count > numRetainedVersions)
            {
                // Leave only the newest version and the previous version.
                foreach (var directory in directories.Take(directories.Count - numRetainedVersions))
                {
